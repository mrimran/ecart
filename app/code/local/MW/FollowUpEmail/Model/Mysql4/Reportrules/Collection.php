...
class MW_FollowUpEmail_Model_Mysql4_Reportrules_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract

{
	...
	{			
			$resource = Mage::getModel('core/resource');
  	  		...
  	  		$rule_table = $resource->getTableName('followupemail/rules');

	        $this->_reset() ->addFieldToFilter('main_table.create_date', array('from' => $from, 'to' => $to, 'datetime' => true));

	        $this->getSelect()->joinLeft(

      							array('rule'=>$rule_table),'main_table.rule_id = rule.rule_id');
	       ...
	       $this->getSelect()->group(array('rule.title'));
